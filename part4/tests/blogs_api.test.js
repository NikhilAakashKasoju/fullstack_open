...
test('when a specific blog is deleted', async () => {
    ...
    const remaningIds = blogsAtEnd.body.map(blog => blog.id)
    assert.strictEqual(remaningIds.includes(blogIdToDelete), false)
})

test('when updating individual inforamtion', async () => {
...
test('updating a non-existent blog returns null in response', async () => {
